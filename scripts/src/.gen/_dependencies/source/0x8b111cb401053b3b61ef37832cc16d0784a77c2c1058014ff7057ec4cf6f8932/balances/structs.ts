...
export interface BalancesFields { inner: ToField<UID>; items: ToField<"u64"> }

export type BalancesReified = Reified< Balances, BalancesFields >;

export class Balances implements StructClass { static readonly $typeName = `${PKG_V1}::balances::Balances`; static readonly $numTypeParams = 0;

 readonly $typeName = Balances.$typeName;

 readonly $fullTypeName: `${typeof PKG_V1}::balances::Balances`;

 readonly $typeArgs: [];

 readonly inner: ToField<UID>; readonly items: ToField<"u64">
 ...
 static reified( ): BalancesReified { return { typeName: Balances.$typeName, fullTypeName: composeSuiType( Balances.$typeName, ...[] ) as `${typeof PKG_V1}::balances::Balances`, typeArgs: [ ] as [], reifiedTypeArgs: [], fromFields: (fields: Record<string, any>) => Balances.fromFields( fields, ), fromFieldsWithTypes: (item: FieldsWithTypes) => Balances.fromFieldsWithTypes( item, ), fromBcs: (data: Uint8Array) => Balances.fromBcs( data, ), bcs: Balances.bcs, fromJSONField: (field: any) => Balances.fromJSONField( field, ), fromJSON: (json: Record<string, any>) => Balances.fromJSON( json, ), fromSuiParsedData: (content: SuiParsedData) => Balances.fromSuiParsedData( content, ), fetch: async (client: SuiClient, id: string) => Balances.fetch( client, id, ), new: ( fields: BalancesFields, ) => { return new Balances( [], fields ) }, kind: "StructClassReified", } }
 ...
 inner: UID.bcs, items: bcs.u64()

}) };

 static fromFields( fields: Record<string, any> ): Balances { return Balances.reified( ).new( { inner: decodeFromFields(UID.reified(), fields.inner), items: decodeFromFields("u64", fields.items) } ) }

 static fromFieldsWithTypes( item: FieldsWithTypes ): Balances { if (!isBalances(item.type)) { throw new Error("not a Balances type");

 }

 return Balances.reified( ).new( { inner: decodeFromFieldsWithTypes(UID.reified(), item.fields.inner), items: decodeFromFieldsWithTypes("u64", item.fields.items) } ) }
 ...
 static fromJSONField( field: any ): Balances { return Balances.reified( ).new( { inner: decodeFromJSONField(UID.reified(), field.inner), items: decodeFromJSONField("u64", field.items) } ) }
 ...
 return Balances.fromBcs( fromB64(res.data.bcs.bcsBytes) ); }
