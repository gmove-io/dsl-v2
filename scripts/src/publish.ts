...
import { client, keypair, IObjectInfo } from './utils.js';
...
(async () => {
	...
	execSync(
		`${process.env.CLI_PATH!} move build --dump-bytecode-as-base64 --path ${process.env.DLAB_PACKAGE_PATH!}`, 
		{ encoding: 'utf-8' }
	));
	...
	try {		
		...
		fs.writeFileSync('./src/created.json', JSON.stringify(objects, null, 2));
